...
import type { FindingGhostbook, Ghostbook } from '@/types/guestbook';

export async function insert(ghostbook: Ghostbook) {
  console.log('ghostbook', ghostbook);
  const res = await db.insert(guestbookSchema).values(ghostbook).returning();
...
export async function page(finding: FindingGhostbook) {
  ...
  const totalResult = await db
    .select({ value: count(guestbookSchema.id) })
    .from(guestbookSchema);
